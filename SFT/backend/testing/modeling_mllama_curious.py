...
class MllamaVisionModel(MllamaPreTrainedModel):
    ...
    def forward(
        ...
        if output_hidden_states:
            print(len(tuple(all_intermediate_hidden_states))) 
            print(len(tuple(global_output[1])))
...
class MllamaTextModel(MllamaPreTrainedModel):
    ...
    def forward(
        ...
        print(f"Intial hidden states {hidden_states.shape}") if curious else None
        ...
        for idx, decoder_layer in enumerate(self.layers):
            ...
            hidden_states = layer_outputs[0]
            # print(f"Hidden states idx {idx}: {hidden_states.shape}") if curious else None
        ...
        print(f"Hidden states post loop: {hidden_states.shape}") if curious else None
...
class MllamaForConditionalGeneration(MllamaPreTrainedModel, GenerationMixin):
    ...
    def __init__(
        ...
        if use_super_vision_model:
            self.vision_model = MllamaSuperVisionModel.from_pretrained(super_vision_path_or_id, config = super_vision_config)
        else:
    ...
    def forward(
        ...
        if pixel_values is not None:
            ...
            ## ALL PRINT STATEMENTS ARE CUSTOM LINES
            cross_attention_states = vision_outputs[0] ; print(cross_attention_states.shape)

            # cross_attention_states = cross_attention_states.mean(dim=1, keepdim=True) ; print(cross_attention_states.shape)
            
            cross_attention_states = self.multi_modal_projector(cross_attention_states).reshape(
                -1, cross_attention_states.shape[-2], self.hidden_size
            ) ; print(cross_attention_states.shape)
        ...
        # print(f"cross_attention_states: {cross_attention_states.shape}")
        # print(f"cross_attention_mask: {cross_attention_mask.shape}")
        # print(f"full_text_row_masked_out_mask: {full_text_row_masked_out_mask}")
        ...
        # decoded_text = processor.decode(predicted_token_ids[0], skip_special_tokens=True)

        # print(outputs.logits.shape , decoded_text) 
