...
class Stage2DatasetHandler:
    ...
    def generate_samples(
        ...
        '''
        Use reservoir sampling to randomly select which samples to process.
        '''
    ...
    def map_bounding_boxes(
        ...
        '''
        Map bounding boxes onto each PDF sample.
        '''
        
        def rescale_list_of_bboxs(
            bboxes:list[list],
            height:int,
            width:int,
        )->list[list[int]]:
        ...
        for sample in self.samples:
            ...
            for page,image in zip(sample.pages,sample.images):
                ...
                images_bboxs=rescale_list_of_bboxs(
                    bboxes=page.images_bbox,
                    height=height,
                    width=width,
                )
                images_bboxs_no_text_overlap=rescale_list_of_bboxs(
                    bboxes=page.images_bbox_no_text_overlap,
                    height=height,
                    width=width,
                )
                lines_bbox=rescale_list_of_bboxs(
                    bboxes=page.lines.bbox,
                    height=height,
                    width=width,
                )
                words_bbox=rescale_list_of_bboxs(
