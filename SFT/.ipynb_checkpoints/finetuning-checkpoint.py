...
def main(
    subhub:bool=False,
    vision_embeddings_only:bool=False,
    **kwargs
)->dict:
    ...
    if config.model_type == "mllama":
        is_vision = True
        # originally uses , MllamaForConditionalGeneration
        model = MllamaProjectManagerForConditionalGeneration.from_pretrained(
        ...
        ) if subhub else MllamaForConditionalGeneration.from_pretrained(
            train_config.model_name,
            quantization_config=bnb_config,
            attn_implementation="sdpa" if train_config.use_fast_kernels else None,
            device_map=(
    ...
    if train_config.use_peft:
        ...
        # Generate the peft config and start fine-tuning from original model
            ...
            prepare_vision_embedding_layers(model) if vision_embeddings_only else None
        if wandb_run:
        ...
        if vision_embeddings_only:
            ...
            freeze_model(model) ; prepare_vision_embedding_layers(model=model,reverse=False)
    ...
    if train_config.enable_fsdp:
        ...
        # Create the FSDP wrapper for vision layers
        if is_vision:
            wrapped_layers = [
                    MllamaSelfAttentionDecoderLayer,
                    MllamaCrossAttentionDecoderLayer,
        ...
        model = FSDP(
            model,
            auto_wrap_policy=(
                my_auto_wrapping_policy if train_config.use_peft or subhub else wrapping_policy
            ),
            cpu_offload=(
    ...
    if vision_embeddings_only and not train_config.use_peft:
        # revert all layers that are not embedding layers to not require grads
        prepare_vision_embedding_layers(model=model,reverse=True)

    count_trainable_params(model,outputs=False,to_print=True)
    ...
    # Debug we were cross-checking weights w/ params require grads with FSDP.
    #
    # We were saving the flattened weights on each gpu to the log directory.
    # pd.DataFrame(rows).to_csv(
    #     f"/home/ubuntu/logs/log_{torch.distributed.get_rank()}.csv" , index=False
    # ) if subhub else pd.DataFrame(rows).to_csv(
    #     f"/home/ubuntu/logs/log_working_{torch.distributed.get_rank()}.csv" , index=False
    # )
